    ...
    return async (req, res, next) => {
        ...
        if (resultJSON.length > 0) {
            resultJSON = resultJSON.map((item) => {
                ...
                copy.filePathShorten = `${rootFolder}${
                    path.sep
                }[...]${diffString(item.filePath, process.cwd())}`;
        ...
        if (type === "full") {
            ...
            req.app.locals.eslint_report = {
                errorCount: eslintReport["frontend"].summary.errorCount + eslintReport["server"].summary.errorCount, 
                warningCount: eslintReport["frontend"].summary.warningCount + eslintReport["server"].summary.warningCount, 
            };
        }
